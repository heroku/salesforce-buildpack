...
const SALESFORCE_URL_REGEX = /^force:\/\/(?:(\w*):(\w*):)?([\w.]*)@([\w@:/\-.]*)$/;
...
const PUSH_SOURCE_OPTIONS = {
    ...
    // API option
    targetusername: WORKSPACE_ALIAS,
    verbose: VERBOSE,
    json: true
};
...
const parseOrgConfigFromUrl = function parseOrgConfigFromUrl(url, username) {
    const match = SALESFORCE_URL_REGEX.exec(url);
    if (match === null) {
        throw new Error(`Invalid SALESFORCE_URL: '${url}'`);
    }
    ...
    return {
        ...
        username: username || WORKSPACE_ORG,
        clientId: match[1] || 'SalesforceDevelopmentExperience',
        clientSecret: match[2] || '1384510088588713504'
    };
};

// Write workspace org config from SALESFORCE_URL to .sfdx to be consumed by push/deploy/test sfdx APIs.
...
const writeOrgConfig = function writeOrgConfig() {
    ...
    return auth(parseOrgConfigFromUrl(SALESFORCE_URL, WORKSPACE_ORG), 'setdefaultusername', WORKSPACE_ORG)
        .then(() => {
            if (DEBUG) {
                utils.info(`Wrote '${WORKSPACE_ORG}' auth config`);
            }
        });
};

// Write hub org config from SALESFORCE_HUB_URL to .sfdx to be consumed by org-create command
const writeHubConfig = function writeHubConfig() {
    ...
    return auth(parseOrgConfigFromUrl(salesforceHubUrl, DEV_HUB_ORG), 'setdefaultdevhubusername', DEV_HUB_ORG)
        .then(() => {
            if (DEBUG) {
                utils.info(`Wrote '${DEV_HUB_ORG}' auth config`);
            }
        });
};

const auth = function auth(orgConfig, setFlag, alias) {
    const orgConfigApi = new force.orgApi();
    orgConfigApi.setName(alias);
    return orgConfigApi.saveConfig(orgConfig)
        .then(() => force.util.getGlobalConfig(`${alias}.json`))
        .then((orgConfigFilePath) => {
            if (DEBUG) {
                utils.info(`Wrote Organization configuration '${alias}' to ${orgConfigFilePath}`);
            }
        });
};

const auth1 = function auth1(authUrl, setFlag, alias) {
    const flags = { sfdxurl: authUrl, setalias: alias };
    flags[setFlag] = true;
