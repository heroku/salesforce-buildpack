...
const MDAPI_DEPLOY_CMD = {
    ...
    flags: {
        ...
        polltimeout: _.getEnvVarValue('SALESFORCE_DEPLOY_POLL_TIMEOUT_MS', false, 3 * 60 * 1000 /* 3 min */),
...
const SALESFORCE_ADDON_POLL_TIMEOUT_MS = _.getEnvVarValue('SALESFORCE_ADDON_POLL_TIMEOUT', false, 4 * 60 * 1000 /* 4 min */);
const SALESFORCE_ADDON_POLL_INTERVAL_MS = _.getEnvVarValue('SALESFORCE_ADDON_POLL_INTERVAL', false, 20 * 1000 /* 20 sec */);
    ...
    function pollConfigVars() {
        return Promise.try(() => {
            ...
            .then(function(response) {
                ...
                if (((new Date()).getTime() - start) > SALESFORCE_ADDON_POLL_TIMEOUT_MS) {
...
const writeOrgConfig = function writeOrgConfig(salesforceUrl) {
    if (DEBUG) _.info(`[DEBUG] ${SALESFORCE_URL_CONFIG_VAR_NAME}: ${salesforceUrl}`);
    orgConfig = getOrgConfigFromUrl(salesforceUrl);
    if (DEBUG) _.info(`[DEBUG] Org config: ${JSON.stringify(orgConfig)}`);
