...
const IS_SALESFORCE_WORKSPACE = _.getEnvVarValue('SALESFORCE_WORKSPACE', true, true, true); // set by bin/compile
...
const SALESFORCE_DIR = _.getEnvVarValue('SALESFORCE_DIR', true); // set by bin/compile
...
const DEFAULT_ORG_SHAPE = {
    ...
    'SignupEmail': `user@${HEROKU_APP_NAME_REPLACE_TOKEN}.com`,
    'Edition': 'Developer',
    'OrgPreferences' : {
        'S1DesktopEnabled' : true
    }
};
const ORG_CONFIG_USERNAME = `org@${HEROKU_APP_NAME_REPLACE_TOKEN}.com`;
const HUB_CONFIG_USERNAME = `hub@${HEROKU_APP_NAME_REPLACE_TOKEN}.com`;
...
const RUN_ALL_TEST_TEST_LEVEL = 'RunAllTestsInOrg';
// FIXME: convert to secs
    ...
    return makeArtifactDir()
        .then(() => {
            if (salesforceUrl) {
                ...
                return waitForOrgProvisioning()
                    .then((tmpSalesforceUrl) => {
                        salesforceUrl = tmpSalesforceUrl;
                        return salesforceUrl;
                    });
            }
        })
...
const waitForOrgProvisioning = function waitForOrgProvisioning() {
    // add-on query config
    const herokuApiEndpoint = _.getEnvVarValue('HEROKU_API', false, 'api.heroku.com');
    const apiToken = _.getEnvVarValue('HEROKU_API_TOKEN', true);
    const addOnName = _.getEnvVarValue('SALESFORCE_ADDON_NAME', false, 'salesforce' /* prod */);

    let cnt = 0;
    let ticks = [];

    _.info(`Waiting for Salesforce Add-on (${addOnName}) to provision org (timeout: ${_.toSec(SALESFORCE_ADDON_POLL_TIMEOUT_MS)}s, interval: ${_.toSec(SALESFORCE_ADDON_POLL_INTERVAL_MS)}s)...`);
    ...
    function pollConfigVars() {
        return Promise.try(() => {
                ...
                ticks.push('-');
                _.info(`${ticks.join('')}>`);
                return queryApi(herokuApiEndpoint, apiToken, 'GET', `/apps/${HEROKU_APP_NAME}/config-vars`);
            })
...
const queryApi = function queryApi(herokuApiEndpoint, apiToken, method, path) {
    const options = {
        host: herokuApiEndpoint,
        path: path,
        method: method,
        headers: {
...
const writeOrgConfig = function writeOrgConfig(salesforceUrl) {
    ...
    orgConfig = parseOrgConfigFromUrl(salesforceUrl, 'workspace',
        ORG_CONFIG_USERNAME.replace(HEROKU_APP_NAME_REPLACE_TOKEN, HEROKU_APP_NAME));
    ...
    return orgConfigApi.saveConfig(orgConfig)
        .then(() => {
            ...
            if (VERBOSE && !tapOnly) _.info(`Wrote '${orgConfig.username}' user org config to ${orgConfigFilePath}`);
        });
};
...
const writeHubConfig = function writeHubConfig() {
    const salesforceHubUrl = _.getEnvVarValue(SALESFORCE_HUB_URL_CONFIG_VAR_NAME, true);
    if (DEBUG) _.info(`[DEBUG] ${SALESFORCE_HUB_URL_CONFIG_VAR_NAME}: ${salesforceHubUrl}`);

    const hubConfig = parseOrgConfigFromUrl(salesforceHubUrl, 'hub',
        HUB_CONFIG_USERNAME.replace(HEROKU_APP_NAME_REPLACE_TOKEN, HEROKU_APP_NAME));
    if (DEBUG) _.info(`[DEBUG] Hub config: ${JSON.stringify(hubConfig)}`);

    const hubConfigApi = new force.hubOrgApi();
    return hubConfigApi.saveConfig(hubConfig)
        .then((result) => {
            if (DEBUG) _.info(`Wrote '${result.username}' hub config.`);
        });
};
...
const createOrg = function pushSource() {
    _.info('');
    _.action('###   C R E A T E   O R G');  // REVIEWME: we may want to create org silently
    ...
    if (!orgShape) {
        ...
        orgShape.SignupEmail = orgShape.SignupEmail.replace(HEROKU_APP_NAME_REPLACE_TOKEN, HEROKU_APP_NAME);
    ...
    if (DEBUG) _.info(`[DEBUG] CreateOrg options: ${JSON.stringify(CREATE_ORG_CMD.flags)}`);
    ...
    return createOrgCmd.validate(CREATE_ORG_CMD.flags)
        ...
        .then((result) => {
            _.info(`CreateOrg completed in ${_.toSec((new Date()).getTime() - start)}s`);

            return evalResult(CREATE_ORG_CMD.name, result)
        })
        .then((result) => {
            if (VERBOSE) {
                _.info(`Created org '${result.username}' [${result.orgId}]`);
            }

            return result;
        })
        .catch((err) => {
            _.error(`CreateOrg failed: ${err.message}`);
            throw err;
        });
};
...
const pushSource = function pushSource(targetOrg) {
    ...
    if (DEBUG) _.info(`[DEBUG] Push options: ${JSON.stringify(PUSH_SOURCE_CMD.flags)}`);
        ...
        .then((result) => {
            if (VERBOSE) {
                ...
                result.PushedSource.forEach(source => {
                    paths.push(`${_.INDENT.LOG}  ${source.path}`);
                });
...
const retrieveZip = function retrieveZip(targetOrg) {
    ...
    if (DEBUG) _.info(`[DEBUG] Retrieve options: ${JSON.stringify(MDAPI_RETRIEVE_CMD.flags)}`);
...
const deploy = function deploy(addtlDeployOptions) {
    ...
    if (DEBUG) _.info(`[DEBUG] Deploy options: ${JSON.stringify(deployOptions)}`);
 ...
 * Stage (non-Scratch Org w/ BYOO config var): create org, push source to org, retrieve zip for release phase deployment
 *
    ...
    return Promise.resolve()
        .then(() => {
            if (IS_BYOO) {
                ...
                _.info('');
                _.action(`Source push performed in release phase script or Procfile.`);
            }
        });
};
 ...
 * Test phase.
 *
 * Test execution - what tests or suites to run or test level - defined in config vars.
 *
 * @returns {Promise.<TResult>}
 */
const test = function test() {
    return prepareEnv()
        .then(() => {
            const testsToRun = _.getEnvVarValue('SALESFORCE_APEX_TESTS', false, undefined);
            if (testsToRun) {
                TEST_CMD.flags.classnames = testsToRun;
            }

            const suitesToRun = _.getEnvVarValue('SALESFORCE_APEX_TEST_SUITES', false, undefined);
            if (suitesToRun) {
                TEST_CMD.flags.suitenames = suitesToRun;
            }

            const testLevel = _.getEnvVarValue('SALESFORCE_APEX_TEST_LEVEL', false, undefined);
            if (testLevel) {
                TEST_CMD.flags.testlevel = testLevel;
            }

            if (!testsToRun && !suitesToRun && !testLevel) {
                TEST_CMD.flags.testlevel = RUN_ALL_TEST_TEST_LEVEL;
            }

            const artifactDir = _.getEnvVarValue('SALESFORCE_APEX_TEST_ARTIFACT_DIR', false, undefined);
            if (artifactDir) {
                TEST_CMD.flags.testartifactdir = artifactDir;
            }
            ...
            return testCmd.execute(TEST_CMD.flags)
                .then((result) => {
                    if (DEBUG) {
                        // no indent as output is to CI processor
                        console.log(`# [DEBUG] Test options: ${JSON.stringify(TEST_CMD.flags)}`);
                    }
