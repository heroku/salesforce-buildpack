...
const queryApi = function queryApi(herokuApiEndpoint, apiToken, method, urlPath) {
    const options = {
    ...
    return new Promise((resolve, reject) => {
        if (DEBUG) {
            _.info(`${options.method}: ${options.host}${options.urlPath}...`);
        }
        ...
        return Promise.try(() => {
            ...
            .then((response) => {
                if (!util.isNullOrUndefined(response)) {
                    if (DEBUG) {
                        _.info(`[DEBUG] Config vars response: ${response}`);
                    }
                ...
                if (duration > SALESFORCE_ADDON_POLL_TIMEOUT_MS) {
                    ...
                    // query again
                    return Promise.try(() =>
                        Promise.delay(SALESFORCE_ADDON_POLL_INTERVAL_MS)
                            .then(pollConfigVars)
                    );
                }
            });
...
const writeOrgConfig = function writeOrgConfig() {
    if (DEBUG && !tapOnly) {
        _.info(`[DEBUG] ${SALESFORCE_URL_CONFIG_VAR_NAME}: ${salesforceUrl}`);
    }
...
const createOrg = function pushSource() {
    ...
    if (DEBUG) {
        _.info(`[DEBUG] CreateOrg options: ${JSON.stringify(CREATE_ORG_CMD.flags)}`);
    }
        ...
        .then((result) => {
            if (VERBOSE) {
                ...
                result.PushedSource.sort((file1, file2) => {
                    const fileName1 = file1.path.toUpperCase();
                    const fileName2 = file2.path.toUpperCase();
...
const setup = function setup() {
    return writeHubConfig()
        .then(writeOrgConfig)
        .then(() => {
            _.info('Setup environment for AppCloud and Force CLI plugin commands.');
        });
};
...
const main = function main() {
    ...
    return Promise.resolve()
        ...
        .finally(() => {
            const done = `${invokePhase ? invokePhase.toUpperCase() : ''} ${(process.exitCode === 0 ? 'SUCCESS!' : 'FAILED!')}  Completed in ${_.toSec((new Date()).getTime() - start)}s`;
            if (!tapOnly) {
                _.info('');
                _.action(`${done}`);
            } else {
