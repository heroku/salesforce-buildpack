...
// Write workspace org config from SALESFORCE_URL to .sfdx to be consumed by appcloud module.
...
// Write hub org config from SALESFORCE_URL to .sfdx to be consumed by appcloud module
...
const pushSource = function pushSource(targetOrg) {
    ...
    const orgConfigApi = new force.scratchOrgApi();
    orgConfigApi.setName(SALESFORCE_ORG);
    const pushApi = new force.pushApi(orgConfigApi);
...
const setupEnv = function setupEnv() {
    ...
    return config.setEnableEncryption(false) // disable encryption to support headless invocation
        ...
        .then(config.setWorkspaceTypeDefault(force.scratchOrgApi.Types.WORKSPACE, SALESFORCE_ORG))
        .then(() => {
            if (DEBUG) {
