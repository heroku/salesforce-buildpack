...
const pushSource = function pushSource(targetOrg, all) {
    ...
    if (targetOrg) {
        ...
        PUSH_SOURCE_CMD.flags.username = targetOrg;
    }
...
const retrieveZip = function retrieveZip(targetOrg) {
    ...
    if (targetOrg) {
        ...
        // TODO: check for ~/.appcloud/<targetOrg>.json
        MDAPI_RETRIEVE_CMD.flags.username = targetOrg;
    }
    ...
    _.info(`Retrieving source metadata zip from Organization '${MDAPI_RETRIEVE_CMD.flags.username}'...`);
    const start = (new Date()).getTime();
                ...
                if (IS_SALESFORCE_WORKSPACE) {
                    ...
                    return prepareEnv()
                        ...
                        .then((result) => {
                            ...
                            return pushSource(userName, true)
                                .then(() => retrieveZip(userName));
                        })
                        .then(() => {
                            try {
